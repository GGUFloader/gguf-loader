# GGUF Loader Documentation

Complete documentation index for GGUF Loader.

## 📖 Quick Links

- **[README](../README.md)** - Project overview and quick start
- **[Installation](installation.md)** - How to install
- **[User Guide](user-guide.md)** - How to use GGUF Loader
- **[FAQ](faq.md)** - Frequently asked questions

## 📚 Documentation Structure

### For Users

| Document | Description |
|----------|-------------|
| [Installation Guide](installation.md) | Complete installation instructions for all platforms |
| [User Guide](user-guide.md) | Full guide to using GGUF Loader features |
| [FAQ](faq.md) | Common questions and answers |

### For Developers

| Document | Description |
|----------|-------------|
| [Addon Development](addon-development.md) | Create custom addons |
| [Architecture](../ARCHITECTURE.md) | Current architecture - FastAPI backend, React frontend, plan-driven LangGraph agent |
| [Feedback System](feedback-system.md) | Set up user feedback |
| [Contributing](../CONTRIBUTING.md) | How to contribute to the project |

### Project Information

| Document | Description |
|----------|-------------|
| [Security Policy](../SECURITY.md) | Security and vulnerability reporting |
| [Code of Conduct](../CODE_OF_CONDUCT.MD) | Community guidelines |
| [License](../LICENSE) | MIT License details |

## 🚀 Getting Started Path

New to GGUF Loader? Follow this path:

1. **[README](../README.md)** - Understand what GGUF Loader is
2. **[Installation Guide](installation.md)** - Install the application
3. **[User Guide](user-guide.md)** - Learn how to use it
4. **[FAQ](faq.md)** - Get answers to common questions

## 🔧 Developer Path

Want to contribute or extend GGUF Loader?

1. **[Contributing](../CONTRIBUTING.md)** - Learn how to contribute
2. **[Addon Development](addon-development.md)** - Create addons
3. **[Feedback System](feedback-system.md)** - Set up feedback

## 📁 Documentation Files

### Repository Root

```
README.md             # Main project overview (current)
QUICK_REFERENCE.md    # Quick reference card
CHANGELOG.md          # Release history
AGENTS.md             # Codebase guide for AI coding agents
ARCHITECTURE.md       # Current architecture (FastAPI + React + agent)
CONTRIBUTING.md       # Contribution guidelines
SECURITY.md           # Security policy
CODE_OF_CONDUCT.MD    # Community guidelines
LICENSE               # MIT License
```

### docs/ Directory

```
docs/
├── README.md               # Archived v2.x hub (see root README)
├── installation.md         # Installation guide
├── user-guide.md          # User manual
├── addon-development.md   # Addon creation guide
├── feedback-system.md     # Feedback setup guide
└── faq.md                 # Frequently asked questions
```

### Build & Launch

```
BUILD_EXE_INSTRUCTIONS.md  # How to build executable
scripts/LAUNCH_README.md   # Launch scripts documentation
```

## 🔍 Finding Information

### By Topic

**Installation**
- [Installation Guide](installation.md)
- [Launch Scripts](../scripts/LAUNCH_README.md)
- [Build Executable](BUILD_EXE_INSTRUCTIONS.md)

**Usage**
- [User Guide](user-guide.md)
- [FAQ](faq.md)

**Development**
- [Addon Development](addon-development.md)
- [Contributing](../CONTRIBUTING.md)
- [Feedback System](feedback-system.md)

**Project Info**
- [README](../README.md)
- [Security](../SECURITY.md)
- [Code of Conduct](../CODE_OF_CONDUCT.MD)

## 📝 Documentation Standards

All documentation follows these principles:

- **Clear and concise** - Easy to understand
- **Well-organized** - Logical structure
- **Up-to-date** - Reflects current version
- **Accessible** - For all skill levels
- **Actionable** - Practical examples

## 🤝 Contributing to Docs

Help improve our documentation:

1. Fix typos and grammar
2. Add missing information
3. Clarify confusing sections
4. Add examples and tutorials
5. Translate to other languages

See [Contributing Guide](../CONTRIBUTING.md) for details.

## 📞 Getting Help

Can't find what you need?

- 📖 Check the [FAQ](faq.md)
- 💬 [GitHub Discussions](https://github.com/GGUFloader/gguf-loader/discussions)
- 🐛 [Report Issues](https://github.com/GGUFloader/gguf-loader/issues)
- 📧 Email: hossainnazary475@gmail.com

---

**Last Updated:** September 2026
**Version:** 2.3.0
