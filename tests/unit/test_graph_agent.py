...
def test_json_repair(tmp_path: Path) -> None:
    ...
    GraphAgent(llm, tmp_path, plan=False, tools=_full_tools(tmp_path), system_prompt='You are a test assistant for unit tests.').process("Create notes.md saying Hello")
...
def test_failure_driven_retry(tmp_path: Path) -> None:
    ...
    out = GraphAgent(llm, tmp_path, plan=False, system_prompt='You are a test assistant for unit tests.').process("Read the file")
...
def test_step_budget(tmp_path: Path) -> None:
    ...
    out = GraphAgent(llm, tmp_path, plan=False, max_steps=3, tools=_full_tools(tmp_path), system_prompt='You are a test assistant for unit tests.').process("Do many things")
...
def test_streaming_final_answer_tokens(tmp_path: Path) -> None:
    ...
    engine = GraphAgent(llm, tmp_path, plan=False, max_steps=3, tools=_full_tools(tmp_path), system_prompt='You are a test assistant for unit tests.')
...
def test_cancel_stops_run(tmp_path: Path) -> None:
    ...
    engine = GraphAgent(llm, tmp_path, plan=False, max_steps=4, system_prompt='You are a test assistant for unit tests.')
    result: dict = {}
...
def test_read_content_reaches_model_context(tmp_path: Path) -> None:
    ...
    GraphAgent(llm, tmp_path, plan=False, system_prompt='You are a test assistant for unit tests.').process("Read vault.txt and tell me the code")
...
def test_list_names_reach_model_context(tmp_path: Path) -> None:
    ...
    GraphAgent(llm, tmp_path, plan=False, system_prompt='You are a test assistant for unit tests.').process("What files exist?")
...
def test_hedged_repeat_terminates_with_answer(tmp_path: Path) -> None:
    ...
    out = GraphAgent(llm, tmp_path, plan=False, system_prompt='You are a test assistant for unit tests.').process("What files are here?")
...
def test_all_stale_repeats_wrap_up_early(tmp_path: Path) -> None:
    ...
    out = GraphAgent(llm, tmp_path, plan=False, max_steps=8, system_prompt='You are a test assistant for unit tests.').process("What files are here?")
    assert len(out["tool_results"]) == 1
...
def test_summarize_directive_forces_full_read(tmp_path: Path) -> None:
    ...
    statuses: list[str] = []
    out = GraphAgent(llm, tmp_path, plan=False, system_prompt='You are a test assistant for unit tests.').process("summarize the workspace", on_status=statuses.append)
    assert out["response"] == "All done."
...
def test_non_summarize_ask_gets_no_directive(tmp_path: Path) -> None:
    ...
    out = GraphAgent(llm, tmp_path, plan=False, system_prompt='You are a test assistant for unit tests.').process("what does a.md say")
    assert out["response"] == "All done."
...
def test_legitimate_reread_after_mutation_allowed(tmp_path: Path) -> None:
    ...
    out = GraphAgent(llm, tmp_path, plan=False, tools=_full_tools(tmp_path), system_prompt='You are a test assistant for unit tests.').process("List, write a file, then list again")
...
def test_checkpoint_resume_across_instances(tmp_path: Path) -> None:
    ...
    e1 = GraphAgent(llm, ws, checkpoint_path=ckpt, tools=_full_tools(ws), plan=False, system_prompt='You are a test assistant for unit tests.')
    ...
    e2 = GraphAgent(llm, ws, checkpoint_path=ckpt, tools=_full_tools(ws), plan=False, system_prompt='You are a test assistant for unit tests.')
