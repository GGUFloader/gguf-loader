...
class ResourceManager:
    ...
    def _detect_deployment_type(self):
        ...
        if getattr(sys, 'frozen', False):
            ...
            # Check if we're running from an installed package. The repo also
            # contains a `ggufloader/` compat package, so a found spec alone is
            # not enough - only treat it as installed when it lives inside
            # site-packages (pip installs land there; the repo checkout never
            # does, even when a venv happens to be under the current directory).
            try:
                ...
                if spec and spec.origin:
                    ...
                    if 'site-packages' in origin or 'dist-packages' in origin:
                        ...
                        # Package resolves inside the project root: repo checkout.
    ...
    def find_config_dir(self) -> str:
        ...
        """
        if self._deployment_type in ['pyinstaller', 'frozen', 'installed_package']:
            # For executables and installed packages, use a user data directory
            ...
            # For development, use local config directory
    ...
    def find_cache_dir(self) -> str:
        ...
        """
        if self._deployment_type in ['pyinstaller', 'frozen', 'installed_package']:
            # For executables and installed packages, use a user cache directory
            if os.name == 'nt':  # Windows
            ...
            # For development, use local cache directory
    ...
    def find_addons_dir(self) -> str:
        ...
        The addons ship inside the ``ggufloader`` package itself, so this
        resolves relative to this module's location in every deployment:
        dev checkout, pip install, and PyInstaller bundle.
    ...
    def get_user_data_dir(self) -> str:
        """
        Return the per-user data directory for a deployed (installed or frozen)
        app - never write app data into site-packages.
        """
        if os.name == 'nt':  # Windows
            base = os.environ.get('APPDATA', '')
            return os.path.join(base, 'GGUFLoader')
        # Unix-like
        return os.path.join(os.path.expanduser('~'), '.ggufloader')
...
def get_user_data_dir() -> str:
    """Get the per-user data directory for deployed apps."""
    return _resource_manager.get_user_data_dir()
