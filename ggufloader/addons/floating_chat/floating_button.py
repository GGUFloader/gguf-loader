...
class FloatingChatButton(QWidget):
    ...
    def _load_icon(self):
        ...
        try:
            ...
            # Fallback: try resource_manager for float.png then icon.ico (handles
            # installed-package layouts where the images live inside the package).
            try:
                import sys
                sys.path.insert(0, str(project_root))
                from ggufloader.resource_manager import find_icon
                for icon_name in ("float.png", "icon.ico"):
